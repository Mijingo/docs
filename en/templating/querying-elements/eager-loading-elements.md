...
This illustrates an _N+1_ query problem: In addition to the query executed to fetch the entries, Craft will execute an additional query for _each_ entry, to find its related asset. So the number of queries needed will be _N_ (the number of entries) _+ 1_ (the initial entries query). If there are 50 entries, this innocent-looking template code will cost the page 51 queries.

All hope is not lost, though. You can solve this with **eager-loading**, using the `with` criteria parameter.

The purpose of the `with` parameter is to tell Craft the sub-elements you’ll need, so it can fetch them all up front, in as few queries as possible.

Here’s how to apply the `with` parameter to our example:
...
This template code requires only 3 queries: one to fetch the entries, one to determine which assets Craft should eager-load, and one to fetch the assets. Then Craft automatically populates the entries with their respective related assets.
...
Take a look at how we assigned the `image` variable in our examples, before and after applying the `with` parameter:
...
When the assets _aren’t_ eager-loaded, `entry.assetsField` returns the related assets once they’re requested (e.g. when `one()` is called).

However when the assets _are_ eager-loaded, `entry.assetsField` gets overwritten with an array of the eager-loaded assets. So `one()`, `all()`, and other methods are not available. Instead you must stick to the standard array syntaxes. In our example, we’re grabbing the first asset with `entry.assetsField[0]`, and we’re using Twig’s _null-coalescing operator_ (`??`) to define a default value (`null`) in case there is no related asset.
...
You can define custom criteria parameters that Craft will apply as it eager-loads elements. Replace its key with an array that has two values: the key, and an object that defines the criteria parameters that Craft should apply.
